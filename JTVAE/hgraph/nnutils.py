...
def avg_pool(all_vecs, scope, dim):
    size = torch.tensor([le for _, le in scope], device=all_vecs.device)
...
def get_accuracy_sym(scores, labels):
    ...
    lab_scores = scores[torch.arange(len(scores), device=scores.device), labels]
...
def create_pad_tensor(alist):
    ...
    for a in alist:
        ...
        a.extend([0] * pad_len)
    return torch.tensor(alist, dtype=torch.int32)

def zip_tensors(tup_list):
    res = []
    tup_list = list(zip(*tup_list))
    for a in tup_list:
        if isinstance(a[0], int):
            res.append(torch.tensor(a))
        else:
...
def index_scatter(sub_data, all_data, index):
    d0, d1 = all_data.size()
    buf = torch.zeros_like(all_data).scatter_(0, index.unsqueeze(1).expand(-1, d1), sub_data)
...
def hier_topk(cls_scores, icls_scores, vocab, topk):
    ...
    batch_index = torch.arange(batch_size, device=cls_topk.device).unsqueeze(1).expand(-1, topk)
