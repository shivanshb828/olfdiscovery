...
class MPNEncoder(nn.Module):
    ...
    def forward(self, fnode, fmess, agraph, bgraph):
        ...
        return node_hiddens * mask, h  # return only the hidden state
...
class HierMPNEncoder(nn.Module):
    ...
    def __init__(self, vocab, avocab, rnn_type, embed_size, hidden_size, depthT, depthG, dropout):
        ...
        self.E_a = torch.eye(atom_size, device=self.device)
        self.E_b = torch.eye(len(MolGraph.BOND_LIST), device=self.device)
        self.E_apos = torch.eye(MolGraph.MAX_POS, device=self.device)
        self.E_pos = torch.eye(MolGraph.MAX_POS, device=self.device)
    ...
    def embed_root(self, hmess, tree_tensors, roots):
        roots = torch.tensor(roots, device=self.device)
...
class IncHierMPNEncoder(HierMPNEncoder):
    ...
    def embed_sub_tree(self, tree_tensors, hinput, subtree, is_inter_layer):
        ...
        if len(submess) == 0:
            ...
            node_buf = torch.zeros(num_nodes, self.hidden_size, device=self.device)
