...
def make_device(tensors, device=None):
    if device is None:
        device = get_device()
...
class HierVAE(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.encoder = HierMPNEncoder(
            args.vocab, args.atom_vocab, args.rnn_type, args.embed_size,
            args.hidden_size, args.depthT, args.depthG, args.dropout
        )
        self.decoder = HierMPNDecoder(
            args.vocab, args.atom_vocab, args.rnn_type, args.embed_size,
            args.hidden_size, args.latent_size, args.diterT, args.diterG, args.dropout
        )
    ...
    def rsample(self, z_vecs, W_mean, W_var, perturb=True):
        ...
        kl_loss = -0.5 * torch.sum(
            1.0 + z_log_var - z_mean.pow(2) - torch.exp(z_log_var)
        ) / batch_size
    ...
    def sample(self, batch_size, greedy):
        ...
        root_vecs = torch.randn(batch_size, self.latent_size, device=device)
    ...
    def reconstruct(self, batch):
        ...
        device = next(self.parameters()).device
        tree_tensors, graph_tensors = tensors = make_device(tensors, device=device)
...
class HierVGNN(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.decoder = HierMPNDecoder(
            args.vocab, args.atom_vocab, args.rnn_type, args.embed_size,
            args.hidden_size, args.hidden_size, args.diterT, args.diterG,
            args.dropout, attention=True
        )
        self.encoder.tie_embedding(self.decoder.hmpn)
    ...
    def translate(self, tensors, num_decode, enum_root, greedy=True):
        device = next(self.parameters()).device
        tensors = make_device(tensors, device=device)
        ...
        if enum_root:
            ...
            root_vecs = torch.cat(
                [root_vecs] * repeat + [root_vecs[:modulo]], dim=0
            )
            tree_vecs = torch.cat(
                [tree_vecs] * repeat + [tree_vecs[:modulo]], dim=0
            )
            graph_vecs = torch.cat(
                [graph_vecs] * repeat + [graph_vecs[:modulo]], dim=0
            )

        batch_size = len(root_vecs)
        z_tree = torch.randn(
            batch_size, 1, self.latent_size, device=device
    ...
    def forward(self, x_graphs, x_tensors, y_graphs, y_tensors, y_orders, beta):
        device = next(self.parameters()).device
        x_tensors = make_device(x_tensors, device=device)
        y_tensors = make_device(y_tensors, device=device)
        ...
        loss, wacc, iacc, tacc, sacc = self.decoder(
            (x_root_vecs, x_tree_vecs, x_graph_vecs),
            y_graphs,
            y_tensors,
            y_orders
        )
...
class HierCondVGNN(HierVGNN):
    ...
    def __init__(self, args):
        ...
        self.U_tree = nn.Sequential(
            nn.Linear(args.hidden_size + args.cond_size, args.hidden_size),
            nn.ReLU()
        )
    ...
    def translate(self, tensors, cond, num_decode, enum_root):
        ...
        cond = cond.view(1, 1, -1).to(device)
        ...
        z_tree = torch.randn(
            num_decode, 1, self.latent_size, device=device
        ).expand(-1, tree_vecs.size(1), -1)
        z_graph = torch.randn(
            num_decode, 1, self.latent_size, device=device
        ).expand(-1, graph_vecs.size(1), -1)
    ...
    def forward(self, x_graphs, x_tensors, y_graphs, y_tensors, y_orders, cond, beta):
        ...
        diff_tree_vecs = y_tree_vecs.sum(dim=1) - x_tree_vecs.sum(dim=1)
        diff_graph_vecs = y_graph_vecs.sum(dim=1) - x_graph_vecs.sum(dim=1)
        diff_tree_vecs = self.U_tree(torch.cat([diff_tree_vecs, cond], dim=-1))
        diff_graph_vecs = self.U_graph(torch.cat([diff_graph_vecs, cond], dim=-1))
        ...
        diff_tree_vecs = torch.cat([diff_tree_vecs, cond], dim=-1)
        diff_graph_vecs = torch.cat([diff_graph_vecs, cond], dim=-1)
