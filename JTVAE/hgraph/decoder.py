...
class HierMPNDecoder(nn.Module):
    ...
    def __init__(self, vocab, avocab, rnn_type, embed_size, hidden_size, latent_size, depthT, depthG, dropout, attention=False):
        ...
        self.E_order = torch.eye(MolGraph.MAX_POS, device=self.device)
    ...
    def update_graph_mask(self, graph_batch, new_atoms, hgraph):
        new_atom_index = torch.tensor(new_atoms, device=self.device)
        ...
        new_bond_index = torch.tensor(new_bonds, device=self.device)
        if len(new_bonds) > 0:
    ...
    def init_decoder_state(self, tree_batch, tree_tensors, src_root_vecs):
        ...
        htree.emask = torch.cat(
            [bgraph.new_zeros(num_mess, device=self.device), bgraph.new_ones(batch_size, device=self.device)],
            dim=0
        )
    ...
    def forward(self, src_mol_vecs, graphs, tensors, orders):
        ...
        hinter = HTuple(
            mess=self.rnn_cell.get_init_state(inter_tensors[1], src_root_vecs),
            emask=self.itensor.new_zeros(inter_tensors[1].size(0), device=self.device)
        )
        hgraph = HTuple(
            mess=self.rnn_cell.get_init_state(graph_tensors[1], src_root_vecs),
            vmask=self.itensor.new_zeros(graph_tensors[0].size(0), device=self.device),
            emask=self.itensor.new_zeros(graph_tensors[1].size(0), device=self.device)
        )
        ...
        for t in range(maxt):
            ...
            subtree = (torch.tensor(subtree[0], device=self.device), torch.tensor(subtree[1], device=self.device))
            ...
            for i in batch_list:
                ...
                if yid is not None:
                    mess_idx = tree_batch[xid][yid]['mess_idx']
                    new_atoms.extend(tree_batch.nodes[yid]['cluster'])  # Regardless of tlab = 0 or 1
                ...
                inter_label = [(pos, self.vocab[(cls, x)][1]) for pos, x in inter_label]
                ...
                if len(tree_batch.nodes[xid]['cluster']) > 2:  # Uncertainty occurs only when previous cluster is a ring
                    ...
                    icls = [self.vocab[(cls, x)][1] for x in inter_label]
                    ...
                    batch_idx = torch.tensor([i] * max_cls_size, device=self.device)
                    assm_scores = self.get_assm_score(src_graph_vecs, batch_idx, cand_vecs).tolist()
                    sorted_cands = sorted(list(zip(cands, assm_scores)), key=lambda x: x[1], reverse=True)

                for inter_label, _ in sorted_cands:
                    inter_label = list(zip(inter_label, attach_points))
                    if graph_batch.try_add_mol(i, self.vocab.get_smiles(clab), inter_label):
                        new_atoms, new_bonds, attached = graph_batch.add_mol(i, self.vocab.get_smiles(clab), inter_label, nth_child)
    ...
    def enum_attach(self, hgraph, cands, icls, nth_child):
        cands = torch.tensor(cands, device=self.device)
        icls_vecs = torch.tensor(icls * len(cands.view(-1)), device=self.device)
        icls_vecs = self.E_assm(icls_vecs)

        nth_child = torch.tensor([nth_child] * len(cands.view(-1)), device=self.device)
    ...
    def decode(self, src_mol_vecs, greedy=True, max_decode_step=100, beam=5):
        ...
        stack = [[] for _ in range(batch_size)]
        ...
        for bid in range(batch_size):
            ...
            root_idx = tree_batch.add_node(torch.tensor([clab, ilab], device=self.device))
        ...
        htree = HTuple(mess=self.rnn_cell.get_init_state(tree_tensors[1], init_vecs))
        hinter = HTuple(mess=self.rnn_cell.get_init_state(tree_tensors[1], init_vecs))
        hgraph = HTuple(mess=self.rnn_cell.get_init_state(graph_tensors[1], init_vecs))
        ...
        for t in range(max_decode_step):
            ...
            batch_idx = torch.tensor(batch_list, device=self.device)
            cur_tree_nodes = [stack[bid][-1] for bid in batch_list]
            subtree = (torch.tensor(cur_tree_nodes, device=self.device), torch.tensor([], device=self.device))
            subgraph = (
                torch.tensor(tree_batch.get_cluster_nodes(cur_tree_nodes), device=self.device),
                torch.tensor(tree_batch.get_cluster_edges(cur_tree_nodes), device=self.device)
            )
            ...
            for i, bid in enumerate(batch_list):
                if topo_preds[i] > 0.5 and tree_batch.can_expand(stack[bid][-1]):
                    ...
                    edge_feature = torch.tensor([stack[bid][-1], new_node, 0], device=self.device)  # Parent to child is 0
                    ...
                    if len(stack[bid]) > 0:
                        ...
                        edge_feature = torch.tensor([child, stack[bid][-1], nth_child], device=self.device)
                        new_edge = tree_batch.add_edge(child, stack[bid][-1], edge_feature)
                        new_mess.append(new_edge)

            subtree = (subtree[0], torch.tensor(new_mess, device=self.device))
            subgraph = ([], [])
            htree, hinter, hgraph = self.hmpn(
                tree_tensors, tree_tensors, graph_tensors, htree, hinter, hgraph, subtree, subgraph
            )
            ...
            if len(expand_list) > 0:
                ...
                idx_in_mess = torch.tensor(idx_in_mess, device=self.device)
                expand_list = torch.tensor(expand_list, device=self.device)
                forward_mess = cur_mess.index_select(0, idx_in_mess)
                cls_scores, icls_scores = self.get_cls_score(src_tree_vecs, expand_list, forward_mess, None)
            ...
            for i, bid in enumerate(expand_list):
                ...
                for kk in cls_beam:  # Try until one is chemically valid
                    ...
                    node_feature = torch.tensor([clab, ilab], device=self.device)
                    ...
                    if len(inter_cands) == 0:
                        ...
                        batch_idx = torch.tensor([bid] * len(inter_cands), device=self.device)
                        assm_scores = self.get_assm_score(src_graph_vecs, batch_idx, cand_vecs).tolist()
