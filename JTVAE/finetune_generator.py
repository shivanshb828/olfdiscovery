        ...
        for root, _, files in os.walk(checkpoint_dir):
            for fname in files:
                if fname.endswith('.pt'):
                    ...
                    model = load_checkpoint(fname, device=self.device)
...
if __name__ == "__main__":
    ...
    score_func = Chemprop(args.chemprop_model, device=device)
    ...
    checkpoint = torch.load(args.generative_model, map_location=device)
    model_state, optimizer_state, _, beta = checkpoint
    ...
    for epoch in range(args.epoch):
        ...
        for _ in range(args.inner_epoch):
            meters = np.zeros(6)
            dataloader = DataLoader(dataset, batch_size=1, collate_fn=lambda x: x[0], shuffle=True, num_workers=0)
